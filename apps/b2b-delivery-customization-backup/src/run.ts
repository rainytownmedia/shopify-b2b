...
export function run(input: RunInput): FunctionRunResult {
    ...
    if (customerTagsStr) {
        try {
            ...
            console.error(`[B2B_DELIVERY_DEBUG] Error parsing customer tags:`, e);
        }
    }
    ...
    console.error(`[B2B_DELIVERY_DEBUG] Subtotal: ${cartTotal} | Qty: ${cartQty} | Tags: ${JSON.stringify(customerTags)}`);

    // 3. Get Shipping Rules from Shop Metafield
    const rulesStr = input.shop.metafield?.value;
    if (!rulesStr) {
        console.error(`[B2B_DELIVERY_DEBUG] No shipping rules found in shop metafield.`);
        return EMPTY_RESULT;
    }
    ...
    try {
        ...
        console.error(`[B2B_DELIVERY_DEBUG] Error parsing shipping rules:`, e);
        return EMPTY_RESULT;
    }
    ...
    rules.forEach(rule => {
        ...
        // Check if rule applies to this customer (by tag)
        ...
        try {
            ...
            console.error(`[B2B_DELIVERY_DEBUG] Error parsing rule conditions:`, e);
        }
        ...
        if (isMatch) {
            console.error(`[B2B_DELIVERY_DEBUG] Match found! Applying rule: ${rule.name}`);
            input.cart.deliveryGroups.forEach(group => {
                group.deliveryOptions.forEach(option => {
                    const shouldHide = rule.targetMethods === "all_paid" 
                        ? true 
                        : rule.targetMethods?.split(",").map((s: string) => s.trim().toLowerCase()).includes(option.title.toLowerCase());
